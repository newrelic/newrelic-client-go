...
func TestSyntheticsSimpleBrowserMonitor_WithMultiBrowserSupport(t *testing.T) {
	...
	match, er := regexp.MatchString("Multi-selection is only supported in the new runtime. Define runtime values.", message)
	...
	match, er = regexp.MatchString("Use only deviceEmulation or devices field.", message)
	require.NoError(t, er)
	require.True(t, match)
	...
	simpleBrowserMonitorInputUpdated := SyntheticsUpdateSimpleBrowserMonitorInput{
		...
		Period:   SyntheticsMonitorPeriodTypes.EVERY_5_MINUTES,
		Status:   SyntheticsMonitorStatus(SyntheticsMonitorStatusTypes.ENABLED),
		Browsers: []SyntheticsBrowser{SyntheticsBrowserTypes.CHROME, SyntheticsBrowserTypes.EDGE},
	...
	require.NoError(t, err)
	require.NotNil(t, updateSimpleBrowserMonitor)
	require.Greater(t, len(updateSimpleBrowserMonitor.Errors), 0)
	message = updateSimpleBrowserMonitor.Errors[0].Description
	match, er = regexp.MatchString("Multi-selection is only supported in the new runtime. Define runtime values.", message)
	require.NoError(t, er)
	require.True(t, match)
...
func TestSyntheticsScriptBrowserMonitor_WithMultiBrowserSupport(t *testing.T) {
	...
	updatedScriptBrowserMonitorInput := SyntheticsUpdateScriptBrowserMonitorInput{
		...
		Status:   SyntheticsMonitorStatusTypes.ENABLED,
		Browsers: []SyntheticsBrowser{SyntheticsBrowserTypes.CHROME, SyntheticsBrowserTypes.EDGE},
		Devices:  []SyntheticsDevice{SyntheticsDeviceTypes.DESKTOP, SyntheticsDeviceTypes.MOBILE_LANDSCAPE},
		Tags: []SyntheticsTag{
			{
				Key: "pineapple",
				Values: []string{
	...
	require.NoError(t, err)
	require.NotNil(t, updateScriptBrowserMonitor)
	require.Greater(t, len(updateScriptBrowserMonitor.Errors), 0)
	message = updateScriptBrowserMonitor.Errors[0].Description
	match, er = regexp.MatchString("Multi-selection is only supported in the new runtime. Define runtime values.", message)
	require.NoError(t, er)
	require.True(t, match)
...
func TestSyntheticsStepMonitor_WithMultiBrowserSupport(t *testing.T) {
	...
	updatedMonitor, err := a.SyntheticsUpdateStepMonitor(createdMonitor.Monitor.GUID, monitorUpdateInput)
	require.NoError(t, err)
	require.NotNil(t, updatedMonitor)
	require.Greater(t, len(updatedMonitor.Errors), 0)
	message = updatedMonitor.Errors[0].Description
	match, er = regexp.MatchString("Multi-selection is only supported in the new runtime. Define runtime values.", message)
	require.NoError(t, er)
	require.True(t, match)
