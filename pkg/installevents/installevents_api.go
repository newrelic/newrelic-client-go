...
func (a *Installevents) InstallationCreateInstallStatusWithContext(
	...
	if len(resp.InstallationCreateInstallStatusResult.Errors) > 0 {
		errs := fmt.Errorf("query error")
		for _, err := range resp.InstallationCreateInstallStatusResult.Errors {
...
func (a *Installevents) InstallationCreateRecipeEventWithContext(
	...
	// If we got errors back, wrap them all up
	if len(resp.InstallationCreateRecipeEventResult.Errors) > 0 {
		errs := fmt.Errorf("query error")
		for _, err := range resp.InstallationCreateRecipeEventResult.Errors {
			errs = fmt.Errorf("%w; %s", errs, err.Description)
		}
		return nil, errs
	}
