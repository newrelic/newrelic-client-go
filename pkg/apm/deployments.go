...
func (apm *APM) ListDeployments(applicationID int) ([]*Deployment, error) {
	...
	for nextURL != "" {
		...
		req, err := http.NewRequest(apm.client, "GET", nextURL, nil, nil, &response)
		if err != nil {
			return nil, err
		}
...
func (apm *APM) CreateDeployment(applicationID int, deployment Deployment) (*Deployment, error) {
	...
	req, err := http.NewRequest(apm.client, "POST", u, nil, &reqBody, &resp)
	if err != nil {
		return nil, err
	}
...
func (apm *APM) DeleteDeployment(applicationID int, deploymentID int) (*Deployment, error) {
	...
	req, err := http.NewRequest(apm.client, "DELETE", u, nil, nil, &resp)
	if err != nil {
		return nil, err
	}
